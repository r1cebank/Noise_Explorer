...
RangeTree::color *RangeTree::insert(unsigned char d)
{
	...
	if(isEmpty()){
		...
		while(curr){
			parent = curr;
			if(t->data > (curr->data + _range))
				curr = curr->right;
			else if (t->data < (curr->data - _range))
				curr = curr->left;
			else {
				cout << "(no allign) t:" << (int)t->data << endl;
				return curr->currentColor;
			}
		}
		//If a color is not found
		if(t->data < (parent->data - _range)){
			//Smaller than range
			/*
				allign the data then add smaller call allign left
			*/
			t->data = allignLeft(t->data, parent->data);
			cout << "(allignLeft) t:" << (int)t->data << endl;
			parent->left = t;
			return t->currentColor;
		}else{
			t->data = allignRight(t->data, parent->data);
			cout << "(allignRight) t:" << (int)t->data << endl;
			parent->right = t;
			return t->currentColor;
		}
	}
